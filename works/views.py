...
def process_work(request):
    ...
    return HttpResponse(message + f" <a href=\"http://localhost:8000/tasks/{task.id}\">a</a>")
...
def check_task(request, task_id):
    ...
    if str(task.status).lower() == "success":
        ...
        message = f"작업이 완료되었습니다.<br><br>- 걸린 시간: {work.elapsed_time}<br>- {work.n} 번째 값: {work.result}"
